...
router.get("/:id", celebrate(taskIdSchema), async (req: Request, res: Response) => {
  try {
    const task = await taskService.findById(parseInt(req.params.id as string));
    if (!task) {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      const task = await taskService.update(parseInt(req.params.id as string), req.body);
      if (!task) {
        res.status(404).json(errorResponse("Task not found"));
        return;
      }
...
router.delete("/:id", celebrate(taskIdSchema), async (req: Request, res: Response) => {
  try {
    const deleted = await taskService.remove(parseInt(req.params.id as string));
    if (!deleted) {
      res.status(404).json(errorResponse("Task not found"));
      return;
    }
